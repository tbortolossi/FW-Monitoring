...
Keep this dashboard API-only: calculate throughput from deltas of the per-port MAC counters `port/rx-bytes` and `port/tx-bytes` returned by `show counter interface all`, with `ibytes` and `obytes` only as a fallback when the `port` block is missing. Amended 2026-09-24: a load test with half its traffic hardware-offloaded showed `ibytes`/`obytes` (and the session kbps summary) at 47 Gb/s while the port counters and SNMP `ifHCInOctets` read 97 Gb/s; the `ibytes`/`obytes` counters are updated by the dataplane and miss offloaded flows. SNMP `ifHCInOctets` returns the same port counter, cached by the SNMP agent for several seconds. Do not use the session throughput summary. The standard dashboards continue using SNMP `ifHCInOctets` and `ifHCOutOctets`.
